Carnet de identidad

Descripción: Carnet de identidad boliviano, debe mostrar  número de CI y fotografía del titular.

Factura de luz

Descripción: Factura de servicio eléctrico domiciliario, debe mostrar nombre del titular y dirección.



Título: Informe de evaluación de proveedor
Contenido inicial:
EVALUACIÓN DE PROVEEDOR
=======================

SECCIÓN LEGAL — Verificación de documentos
-------------------------------------------
Documentos recibidos: 
Observaciones legales: 
Estado: [ ] Aprobado  [ ] Observado  [ ] Rechazado
Responsable: 
Fecha: 

SECCIÓN COMERCIAL — Revisión de referencias
--------------------------------------------
Referencias verificadas: 
Historial comercial: 
Condiciones de pago propuestas: 
Estado: [ ] Aprobado  [ ] Observado  [ ] Rechazado
Responsable: 
Fecha: 

CONCLUSIÓN CONJUNTA
--------------------
Recomendación final:


CLIENTE:
 MENSAJE: "Quiero registrarme como proveedor"

Voz — IA:
 MENSAJE: "La actividad fue completada satisfactoriamente. Se verificaron todos los documentos del proveedor incluyendo carnet de identidad y factura de luz. Como observación, el carnet presentaba fecha de vencimiento próxima pero sigue siendo válido."


REPORTES:
Básicos:

trámites completados este mes
trámites demorados
tareas pendientes del departamento Revisión

Con filtro de cliente:

trámites de Juan Perez
tareas de Carlos Mendez

Con fecha:

trámites creados esta semana
trámites de abril

Para el gráfico:

trámites agrupados por estado → debería devolver tipo_grafico: pie
tareas por departamento → debería devolver tipo_grafico: bar






